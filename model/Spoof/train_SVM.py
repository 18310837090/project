    ...
    for root, dirs, files in os.walk(dir):
        for file in files:
            ...
            if filesuffix in suffix:  # 遍历找到指定后缀的文件名["jpg",png]等
                # print filepath
                if 'inf' in file:
                    inf_face = cv2.imread(filepath)
                    rgb_face = cv2.imread(filepath.replace('inf','rgb'))
                else:
                    continue

                # cv2.imshow('rgb',rgb_face)
                # cv2.imshow('inf',inf_face)
                # cv2.waitKey()
                # 获取频谱特征
                if use_fft:
                    ...
                    inf_face_fft = cv2.cvtColor(inf_face,cv2.COLOR_BGR2GRAY)
                    inf_magnitude_spectrum, inf_FSFeathrer, inf_FSFeathrerMean = getFS(inf_face_fft)

                    fs_lbp = np.hstack((rgb_FSFeathrerMean, inf_FSFeathrerMean,dtype))
                if use_lbp:
                # 获取LBP特征
                    rgb_lbp_hist = getLbp_YCrCb(rgb_face)
                    inf_lbp_hist = getLbp_YCrCb(inf_face)
                    fs_lbp = np.hstack((rgb_lbp_hist,inf_lbp_hist, dtype))
                ...
                if use_glcm:
                    ...
                    inf_face_glcm = cv2.cvtColor(inf_face, cv2.COLOR_BGR2GRAY)
                    inf_glcm_0 = getGlcm(inf_face_glcm, 1, 0)
                    asm1, con1, eng1, idm1 = feature_computer(inf_glcm_0)

                    fs_lbp = np.hstack((asm0, con0, eng0, idm0, asm1, con1, eng1, idm1, dtype))
                    print len(fs_lbp)
...
def trainSVM(model_path, use_fft, use_lbp, kernel,use_glcm):
    ...
    if use_lbp:
        x, y = np.split(trainTestData, (512,), axis=1)
    # if use_lbp and use_fft:
...
def TakeFace(model_path):
    ...
    cap_inf = cv2.VideoCapture(2)
    cap_inf.set(cv2.CAP_PROP_FRAME_WIDTH, size[1])
    cap_inf.set(cv2.CAP_PROP_FRAME_HEIGHT, size[0])
    model = joblib.load(model_path)
    while (cap_rgb.isOpened() and cap_inf.isOpened()):

        ret_rgb, frame_rgb = cap_rgb.read()
        ret_inf, frame_inf = cap_inf.read()
        if ret_rgb == True:
            frame_rgb = np.rot90(frame_rgb, 3)
            frame_inf = np.rot90(frame_inf, 3)

            face_rgb = getFace(frame_rgb)
            face_inf = getFace(frame_inf)

            cv2.imshow('rgb', frame_rgb)
            cv2.imshow('inf', frame_inf)
            if cv2.waitKey(1):# & 0xFF == ord('q'):
                if face_rgb is not None and face_inf is not None:
                    ...
                    # start = time.clock()
                    test_image(model, face_rgb, face_inf, use_lbp=True, use_fft=False, use_glcm=False)
                    # end = time.clock()
                    # print("Running time is : %.03f seconds" % (end - start))
                elif face_inf is None:
                    print 'No face '
        else:
            break
...
def feature_computer(p):
    ...
    for i in range(gray_level):
        for j in range(gray_level):
            ...
            Asm += p[i][j] * p[i][j]
            Idm += p[i][j] / (1 + (i - j) * (i - j))
            if p[i][j] > 0.0: Eng += p[i][j] * np.log(p[i][j])
    return Asm, Con, -Eng, Idm


def test_image(model, rgb_face, inf_face, use_fft, use_lbp, use_glcm):
    # 获取频谱特征
    if use_fft:
        rgb_face_fft = cv2.cvtColor(rgb_face, cv2.COLOR_BGR2GRAY)
        rgb_magnitude_spectrum, rgb_FSFeathrer, rgb_FSFeathrerMean = getFS(rgb_face_fft)
        ...
        fs_lbp = np.hstack((rgb_FSFeathrerMean, inf_FSFeathrerMean))

    if use_lbp:
        # 获取LBP特征
        rgb_lbp_hist = getLbp_YCrCb(rgb_face)
        inf_lbp_hist = getLbp_YCrCb(inf_face)
        fs_lbp = np.hstack((rgb_lbp_hist, inf_lbp_hist))
    ...
    if use_glcm:
        ...
        fs_lbp = np.hstack((asm0, con0, eng0, idm0, asm1, con1, eng1, idm1))
    ...
    cv2.imshow('rgb_face', rgb_face)
    cv2.imshow('inf_face', inf_face)
    # cv2.waitKey()
        ...
        for file in files:
            ...
            if filesuffix in suffix:  # 遍历找到指定后缀的文件名["jpg",png]等
                ...
                if 'inf' in file:
                    ...
                    continue
                test_image(model, rgb_face, inf_face, use_lbp=True, use_fft=False, use_glcm=False)


if __name__ == '__main__':
    # trainSVM('rbf.model',use_fft=False, use_lbp=True, kernel='rbf',use_glcm=False)
    # model_path = './NCITSpoof_model/0.8_16emm.model'
    model_path = './models/0.2_4rbf.model'
    # data_path = './NCITSpoof/test/ImposterFace'
